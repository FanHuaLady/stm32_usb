...
void HAL_PCD_MspInit(PCD_HandleTypeDef* pcdHandle)
{
  ...
  if(pcdHandle->Instance==USB_OTG_HS)												// HS
  {
    ...
    HAL_PWREx_EnableUSBVoltageDetector();											// 使能USB电压检测器
    ...
    GPIO_InitStruct.Alternate = GPIO_AF10_OTG1_HS;									// 复用功能改为AF10
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

    
    __HAL_RCC_USB_OTG_HS_CLK_ENABLE();												// 使能HS时钟
...
{
	if (pdev->id == DEVICE_HS) 														// 增加对HS的支持
	{
		hpcd_USB_OTG_HS.pData = pdev;												// 换成HS
		pdev->pData = &hpcd_USB_OTG_HS;

		hpcd_USB_OTG_HS.Instance = USB_OTG_HS;										// 改为HS外设
		hpcd_USB_OTG_HS.Init.dev_endpoints = 9;										//
		hpcd_USB_OTG_HS.Init.speed = PCD_SPEED_FULL;								// 速度设为全速
		hpcd_USB_OTG_HS.Init.dma_enable = DISABLE;
		hpcd_USB_OTG_HS.Init.phy_itface = PCD_PHY_EMBEDDED;							// 使用内部PHY
		...
		if (HAL_PCD_Init(&hpcd_USB_OTG_HS) != HAL_OK)								// 错
		{
			Error_Handler( );
		}
...
#endif
		HAL_PCDEx_SetRxFiFo(&hpcd_USB_OTG_HS, 0x80);								// 可设置
